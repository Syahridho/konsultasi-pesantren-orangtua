...
const DEFAULT_SETTINGS = {
  ...
  keterangan: "Pembayaran Uang Masuk (1x bayar) santri baru.",
};

// GET: Fetch list of Uang Masuk bills
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Fetch all users to enrich santri & parent info
    const usersRef = ref(database, "users");
    const usersSnapshot = await get(usersRef);
    const users = usersSnapshot.exists() ? usersSnapshot.val() : {};

    // Fetch all tagihan uang masuk
    const uangMasukRef = ref(database, "tagihan_uang_masuk");
    const snapshot = await get(uangMasukRef);
    ...
    console.error("[UANG MASUK GET API] Error:", error);
    return addCorsHeaders(
...
// POST: Create single or bulk tagihan uang masuk
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { action, nominal, santriId, tahun, keterangan, rincianBiaya } = body;

    // Fetch settings for default nominal
    const settingsRef = ref(database, "settings/uang_masuk");
    const settingsSnapshot = await get(settingsRef);
    const settings = settingsSnapshot.exists()
      ? { ...DEFAULT_SETTINGS, ...settingsSnapshot.val() }
    ...
    // 1. Bulk generate for all santri who do NOT have an entry fee bill yet
    if (action === "bulk_generate") {
      ...
      // Check existing uang masuk bills
      const uangMasukRef = ref(database, "tagihan_uang_masuk");
      const snapshot = await get(uangMasukRef);
      ...
      for (const santri of santriList) {
        // One-time entry fee: check if santri already has an entry fee bill
        ...
        const newBillRef = push(uangMasukRef);
        ...
        const billData = {
          ...
          tahun: currentYear,
          nominal: defaultNom,
          rincianBiaya: rincianBiaya || settings.rincianBiaya,
          status: "belum_bayar",
          keterangan:
            keterangan ||
            `Tagihan Uang Masuk (1x Bayar) Santri Baru: ${santri.name}`,
      ...
      return addCorsHeaders(
        NextResponse.json({
          message: `Berhasil membuat ${createdList.length} tagihan uang masuk (${skippedCount} sudah ada)`,
    ...
    // Check if this santri already has a bill
    const snapshot = await get(uangMasukRef);
    if (snapshot.exists()) {
      ...
      if (duplicate) {
        return NextResponse.json(
          {
            error: `Santri ${santri.name} sudah memiliki tagihan uang masuk sebelumnya.`,
          },
          { status: 400 }
        );
      }
    ...
    return addCorsHeaders(
      NextResponse.json({
        message: "Tagihan uang masuk berhasil dibuat",
        tagihan: billData,
      })
    );
  } catch (error) {
    console.error("[UANG MASUK POST API] Error:", error);
    return addCorsHeaders(
      NextResponse.json({ error: "Internal server error" }, { status: 500 })
    );
  }
...
// PUT: Update tagihan uang masuk (submit payment, verify, edit)
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const billRef = ref(database, `tagihan_uang_masuk/${id}`);
    const billSnapshot = await get(billRef);

    if (!billSnapshot.exists()) {
      return NextResponse.json(
        { error: "Tagihan uang masuk tidak ditemukan" },
        { status: 404 }
      );
    }
    ...
    if (action === "submit_pembayaran") {
      ...
      const updateData = {
        buktiPembayaran,
        buktiFileName: buktiFileName || "bukti_uang_masuk.jpg",
      ...
      return addCorsHeaders(
        NextResponse.json({
          message:
            "Bukti pembayaran uang masuk berhasil diunggah. Menunggu verifikasi petugas.",
    ...
    if (action === "verifikasi") {
      ...
      return addCorsHeaders(
        NextResponse.json({
          message:
            newStatus === "lunas"
              ? "Pembayaran uang masuk berhasil disetujui (Lunas)"
              : "Pembayaran uang masuk telah ditolak",
          tagihan: { ...existingBill, ...updateData },
        })
      );
    }
    ...
    const { nominal, tahun, keterangan, rincianBiaya, status: manualStatus } = body;
    ...
    if (rincianBiaya !== undefined) updateData.rincianBiaya = rincianBiaya;
    ...
    return addCorsHeaders(
      NextResponse.json({
        message: "Data tagihan uang masuk berhasil diperbarui",
        tagihan: { ...existingBill, ...updateData },
      })
    );
  } catch (error) {
    console.error("[UANG MASUK PUT API] Error:", error);
    return addCorsHeaders(
      NextResponse.json({ error: "Internal server error" }, { status: 500 })
    );
  }
...
// DELETE: Remove a tagihan uang masuk
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    return addCorsHeaders(
      NextResponse.json({ message: "Tagihan uang masuk berhasil dihapus" })
    );
  } catch (error) {
    console.error("[UANG MASUK DELETE API] Error:", error);
    return addCorsHeaders(
      NextResponse.json({ error: "Internal server error" }, { status: 500 })
    );
  }
